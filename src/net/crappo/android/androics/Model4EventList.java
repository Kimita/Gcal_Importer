...
public class Model4EventList {
    ...
    public void mapSort() {
        ...
        // Mapにputし終わったら、entrySetとしてArrayListに与えてCollection使ってValue比較で降順sort(開始日時の新しい順)する
        ...
        Calendar sortedCalendar = new Calendar(); // 容れ物を用意しておく
        for(Object propObj : calendar.getProperties()) { // まずはVCALENDARのProperty部分を容れ物に詰める
            Property calHeader = (Property)propObj;
            sortedCalendar.getProperties().add(calHeader);
        }
        for(Object compObj : componentBuffer) { // バッファにとっておいたVEVENT以外の要素を容れ物に詰める
            component = (Component)compObj;
            sortedCalendar.getComponents().add(component);
        }
        for(Entry<String, String> entry: entries ) { // 開始日時の新しい順にsortされたuidを使ってEVENTを拾っていく
            Component compoTmp = null;
            for(Object eventObj : calendar.getComponents("VEVENT")) {
                event = (VEvent)eventObj;
                if(entry.getKey().equals(event.getUid().getValue())) { // EntryのuidとVEventのuidが一致したら、VEventを容れ物に詰める
